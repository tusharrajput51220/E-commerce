            ...
            <div className="w-max h-full flex transition-all ease-in-out duration-1000">
                {slides?.map(slide => (
                    <div className={`${slide?.bg} w-screen h-full flex flex-col gap-16 xl:flex:row`} key={slide.id}>
                        {/* text container */}
                        <div className="h-1/2 w-1/2 flex flex-col items-center justify-center gap-8 2xl:gap-12 text-center">
                            <h2 className="text-xl lg:text-3xl 2xl:text-2xl">{slide?.description}</h2>
